...
export function validateConfig(config: AuthConfig): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (isProduction && config.authUrl?.includes('localhost')) {
      console.warn(
        '[auth-vue] Warning: authUrl contains "localhost" but app appears to be in production. ' +
    ...
    if (config.authUrl?.endsWith('/')) {
      console.warn(
        '[auth-vue] Warning: authUrl has a trailing slash. ' +
    ...
    if (config.appUrl?.endsWith('/')) {
      console.warn(
        '[auth-vue] Warning: appUrl has a trailing slash. ' +
          'This may cause double slashes in URLs.',
      );
    }
  }
